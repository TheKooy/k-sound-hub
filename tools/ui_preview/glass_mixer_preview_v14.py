...
class SoundPadCard(QFrame):
    def __init__(self, name: str, icon: str, meta: str, emoji_callback=None):
        super().__init__()
...
class PadsPanel(QWidget):
    ...
    def _position_emoji_overlay(self) -> None:
        if not hasattr(self, "emoji_overlay"):
            return
        width = min(248, max(190, self.width() - 24))
        height = 150
    ...
    def _add_pad(self, name: str | None = None, icon: str = "🎧", meta: str = "new") -> None:
        ...
        card = SoundPadCard(name, icon, meta, emoji_callback=self._show_emoji_palette)
...
class Drawer(QFrame):
    ...
    def _eq_page(self) -> QWidget:
        ...
        channel = SelectButton(["ALL", "GAME", "MEDIA", "CHAT", "MORE", "MICRO", "MIC OUT"], "GAME")
        root.addWidget(channel)

        root.addWidget(QLabel("Preset"))
        preset = SelectButton(["Default", "KSH Neutral", "KSH Game", "KSH Media", "KSH Chat"], "KSH Chat")
