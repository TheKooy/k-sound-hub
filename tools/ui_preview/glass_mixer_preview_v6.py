...
QFrame#titleBar {
    ...
    border-bottom: 1px solid rgba(150, 205, 255, 24);
}
...
class PreviewWindow(QMainWindow):
    ...
    def mousePressEvent(self, event) -> None:
        if event.button() == Qt.LeftButton:
            edges = self._edges_for_pos(event.position().toPoint())
            if edges:
    ...
    def _edges_for_pos(self, pos: QPoint) -> set[str]:
        margin = 8
        rect = self.rect()
        edges: set[str] = set()
