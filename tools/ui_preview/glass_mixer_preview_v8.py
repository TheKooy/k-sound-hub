...
class LevelMeter(QWidget):
    ...
    def tick(self) -> None:
        if self.target > self.current:
            self.current = self.current * 0.35 + self.target * 0.65
        else:
            self.current = self.current * 0.86 + self.target * 0.14
    ...
    def paintEvent(self, event) -> None:
        ...
        for i in range(segments):
            ...
            if i < active:
                if i >= int(segments * 0.82):
                    color = QColor(255, 178, 70, 205)
                elif i >= int(segments * 0.66):
                    color = QColor(90, 235, 145, 190)
                else:
                    color = QColor(70, 210, 255, 165)
            else:
                color = QColor(12, 25, 38, 168)

            painter.setPen(QColor(110, 170, 220, 34))
            painter.setBrush(color)
            painter.drawRoundedRect(seg, 1.6, 1.6)

        peak_y = rect.bottom() - (peak_index + 1) * segment_h - peak_index * gap
        painter.setPen(Qt.NoPen)
        painter.setBrush(QColor(225, 250, 255, 230))
...
class ChannelCard(QFrame):
    def __init__(self, name: str, icon: str, devices: list[str], value: int):
        ...
        meter_row.addWidget(self.right_meter, 0, Qt.AlignBottom)
        meter_row.addStretch(1)
...
"""
    ...
    def _refresh_background(self) -> None:
        ...
        self.background_label.setPixmap(self._saturate_pixmap(scaled))
