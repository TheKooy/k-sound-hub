...
class AppRouteCard(QFrame):
    ...
    def __init__(self, icon: str, name: str, meta: str, target: str):
        ...
        combo.currentTextChanged.connect(self._channel_changed)
...
class SoundPadCard(QFrame):
    ...
    def __init__(self, name: str, icon: str, meta: str):
        super().__init__()
        ...
        self.icon_button.clicked.connect(self._toggle_emoji_palette)
        ...
        self.emoji_palette = QFrame()
        self.emoji_palette.setObjectName("emojiPalette")
        palette_layout = QGridLayout(self.emoji_palette)
        palette_layout.setContentsMargins(5, 5, 5, 5)
        palette_layout.setHorizontalSpacing(3)
        palette_layout.setVerticalSpacing(3)
        ...
        for index, emoji in enumerate(self.EMOJIS):
            ...
            palette_layout.addWidget(button, index // 6, index % 6)

        self.emoji_palette.setVisible(False)
        root.addWidget(self.emoji_palette)
    ...
    def _toggle_emoji_palette(self) -> None:
        if not self._edit_enabled:
            return
        self.emoji_palette.setVisible(not self.emoji_palette.isVisible())
    ...
    def _update_compact_height(self) -> None:
        if not self._edit_enabled:
            self.setMinimumHeight(70)
            return
        self.setMinimumHeight(182 if self.emoji_palette.isVisible() else 100)
...
class PadsPanel(QWidget):
    ...
    def __init__(self, detach_callback=None, columns: int = 2):
        super().__init__()
...
class DetachedPadsWindow(QMainWindow):
    def __init__(self):
        ...
        panel = PadsPanel(detach_callback=None, columns=4)
...
class Drawer(QFrame):
    ...
    def _detach_pads(self) -> None:
        if not hasattr(self, "_detached_pads_windows"):
            self._detached_pads_windows = []
        window = DetachedPadsWindow()
        self._detached_pads_windows.append(window)
        window.show()
