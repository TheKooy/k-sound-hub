...
class PipeWireAudioEngine(PipeWireAudioEngineBase):
    """
    Playback V2 tranche 2.

    Différence principale vs T1:
    - ALL reste vivant comme canal de fallback / défaut.
    - GAME / CHAT / MEDIA / MORE ne gardent leur EQ playback active
      que s'ils portent réellement un flux externe.
    - les meter probes playback inactifs sont stoppés.
    """
    ...
    def _apply_eq_slot(self, settings: AppSettings, key: str) -> None:
        ...
        if proc.poll() is None:
            ...
            if key == "all":
                slot.status = f"active ({profile.name} → {target_label} via {device_bus}, default){suffix}"
            else:
