...
IDLE_STOP_GRACE_S = 4.0
EQ_RECONFIG_DEBOUNCE_S = 0.30
...
class PipeWireAudioEngine(PipeWireAudioEngineBase):
    ...
    def __init__(self) -> None:
        ...
        self._playback_hold_until: dict[str, float] = {}
        self._last_eq_apply_request: dict[str, float] = {}
    ...
    def _note_eq_apply_request(self, key: str) -> None:
        self._last_eq_apply_request[key] = time.monotonic()

    def _eq_reconfig_ready(self, key: str) -> bool:
        requested_at = self._last_eq_apply_request.get(key)
        if requested_at is None:
            return True
        return (time.monotonic() - requested_at) >= EQ_RECONFIG_DEBOUNCE_S

    def _update_playback_hold(self, logical_sink: str, active: bool) -> bool:
        now = time.monotonic()
        if active:
            self._playback_hold_until[logical_sink] = now + IDLE_STOP_GRACE_S
            return True
        hold_until = self._playback_hold_until.get(logical_sink, 0.0)
    ...
    def _playback_channel_should_run(self, settings: AppSettings, key: str, logical_sink: str) -> bool:
        ...
        if channel is None or not channel.enabled:
            return False
        external_active = self._logical_sink_external_stream_count(logical_sink) > 0
        return self._update_playback_hold(logical_sink, external_active)
    ...
    def _apply_eq_slot(self, settings: AppSettings, key: str) -> None:
        ...
        channel_should_run = self._playback_channel_should_run(settings, key, logical_sink)
        if not channel_should_run:
            self._stop_slot(slot)
        ...
        if not self._eq_reconfig_ready(key) and slot.proc is not None and slot.proc.poll() is None:
            slot.status = f"active ({profile.name} → {target_label} via {device_bus}, settling)"
            return
    ...
    def apply_channel(self, settings: AppSettings, channel_key: str) -> None:
        ...
        if channel_key in PLAYBACK_EQ_CHANNELS:
            self._note_eq_apply_request(channel_key)
    ...
    def apply_settings(self, settings: AppSettings) -> None:
        self._last_settings = settings
        for key in PLAYBACK_EQ_CHANNELS:
            self._note_eq_apply_request(key)
